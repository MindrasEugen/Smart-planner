...
import { useBirthdayForm, birthdayTimeUnitOptions } from './useBirthdayForm.js';

/**
 * @typedef {import('./useBirthdayForm.js').UseBirthdayFormProps} UseBirthdayFormProps
 */

export default function BirthdayForm({ item, mode }) {
  const {
    formData,
    errors,
    importanceOptions,
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-surface-container-lowest rounded-xl p-md mb-lg">
      <h2 className="font-headline-md text-on-surface mb-lg">
        {mode === 'edit' ? 'Modifica Compleanno' : 'Crea Nuovo Compleanno'}
      </h2>
      ...
      <div className="flex flex-col gap-md">
        ...
        <div className="flex flex-col md:flex-row gap-md">
          <div className="flex-grow">
            <FormField
              ...
              onChange={handleChange}
              error={errors.title}
              placeholder="Es. Compleanno di Mario"
            />
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row gap-md">
          <div className="flex-grow">
            <FormField
              ...
              placeholder="Descrizione opzionale del compleanno"
            />
          </div>
        </div>
        ...
        <div className="flex justify-end gap-3 pt-3 border-t border-outline-variant mt-1">
          ...
          <button
            type="submit"
            className="px-4 py-2 bg-primary hover:brightness-90 rounded-xl text-on-primary font-label-sm transition-colors active:scale-95"
            aria-label="Salva compleanno"
          >
            Salva
          </button>
        </div>
