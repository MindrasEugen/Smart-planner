...
export default React.memo(function AgendaItemCard({ item }) {
  ...
  return (
    <div
      className={`bg-surface-container-lowest border border-outline-variant rounded-lg relative overflow-hidden flex flex-col ${isCompleted ? 'opacity-60' : ''} hover:bg-surface-container-low transition-colors`}
    >
      {/* Strip colorato verticale a sinistra */}
      <div className={`absolute left-0 top-0 bottom-0 w-1 ${stripColor}`} />

      <div
        className="p-md flex gap-md items-center active:bg-surface-container-high transition-colors cursor-pointer"
        role="article"
      ...
      >
        ...
        {/* Apre/chiude il pannello azioni sotto la card: bottone sempre visibile,
            funziona identico con mouse e touch (niente hover, mai richiesto su mobile) */}
        <button
          type="button"
      ...
      {/* Pannello azioni: espande l'altezza della card invece di sovrapporsi al testo */}
      {showActions && (
